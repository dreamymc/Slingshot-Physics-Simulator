...
pygame.display.set_caption("Slingshot — final (patched + angle indicator)")
...
# helper: compute angle degrees from world pull vector (pwx, pwy)
def angle_from_pull(pwx, pwy):
    # uy positive = up in world coords. atan2 expects (y, x).
...
while running:
    ...
    if last_pull is not None:
        ...
        # draw angle indicator for last_pull
        angle_deg = angle_from_pull(pwx, pwy)
        draw_text(ax_s+12, ay_s-36, f"Angle: {angle_deg:.1f}°", small)
        line_len_px = 36
        end_x = int(ax_s + math.cos(math.radians(angle_deg)) * line_len_px)
        end_y = int(ay_s - math.sin(math.radians(angle_deg)) * line_len_px)
        pygame.draw.line(screen, (220,180,80), (ax_s, ay_s), (end_x, end_y), 3)
        pygame.draw.circle(screen, (220,180,80), (end_x, end_y), 4)
    ...
    if dragging:
        ...
        # compute angle in degrees (0 = right, 90 = up, 180 = left, 270 = down)
        angle_deg = angle_from_pull(pull_wx, pull_wy)

        # draw numeric angle and direction indicator
        draw_text(ax_s+12, ay_s-36, f"Angle: {angle_deg:.1f}°", small)
        line_len_px = 36
        end_x = int(ax_s + math.cos(math.radians(angle_deg)) * line_len_px)
        end_y = int(ay_s - math.sin(math.radians(angle_deg)) * line_len_px)
    ...
    if projectile is None:
        ...
        if is_mouse_near(mx, my, sx_anchor, sy_anchor, r=26):
            if last_pull is not None:
                ...
                # draw angle indicator for last_pull when hovering anchor
                angle_deg = angle_from_pull(pwx, pwy)
                draw_text(ax_s+12, ay_s-36, f"Angle: {angle_deg:.1f}°", small)
                line_len_px = 36
                end_x = int(ax_s + math.cos(math.radians(angle_deg)) * line_len_px)
