# slingshot_fixed_ui_zoom_fixed_ground.py
# pip install pygame
# Run: python slingshot_fixed_ui_zoom_fixed_ground.py
...
pygame.display.set_caption("Slingshot — fixed ground & zoom")
...
# Only horizontal camera offset allowed. Vertical offset is fixed so ground (y=0) stays at same screen row.
cam_off_x_m = 0.0
cam_off_y_m = 0.0  # fixed; DO NOT change this elsewhere (keeps ground fixed)
...
# Initial pixel anchor (same as your old ANCHOR pixel position)
INIT_ANCHOR_PX = (150, HEIGHT - GROUND_H_PX - 20)
ANCHOR_W = screen_to_world(*INIT_ANCHOR_PX)  # anchor in world coords
if ANCHOR_W[1] < 0.0:
...
def draw_grid(surface, meters_between_lines=1, label_every=5, 
    ...
    """Draw grid lines in world meters; horizontal lines start at y=0 (ground) and go upward only."""
...
while running:
    ...
    for ev in pygame.event.get():
        if ev.type == pygame.QUIT:
            ...
            # left click start drag if near anchor (use screen coords of anchor)
            if ev.button == 1:
            ...
            # right click move anchor (set anchor world pos from click). Clamp anchor y >= 0.
            elif ev.button == 3:
                if my < HEIGHT - GROUND_H_PX - 6:
                    new_anchor = screen_to_world(mx, my)
                    # clamp y so anchor never goes below ground
                    if new_anchor[1] < 0.0:
            ...
            # Zoom centered on mouse position horizontally; keep vertical camera fixed so ground stays.
            mx, my = pygame.mouse.get_pos()
            # compute world coord under mouse before zoom
            ...
            if abs(new_ppm - PIXELS_PER_M) > 1e-9:
                old_ppm = PIXELS_PER_M
                PIXELS_PER_M = new_ppm
                # adjust horizontal camera offset so x under mouse remains the same
                cam_off_x_m = world_before[0] - (mx / PIXELS_PER_M)
                # do NOT change cam_off_y_m -> ground fixed at y=0
    ...
    if projectile and projectile["alive"]:
        ...
        if projectile['y'] <= 0.0:
            if prev_state is not None:
                landing = record_landing_exact(prev_state, projectile, ANCHOR_W[0])
            else:
                landing = {'x': projectile['x'], 'speed': math.hypot(projectile['vx'], projectile['vy']), 'range': projectile['x'] - ANCHOR_W[0], 'screen': world_to_screen(projectile['x'], 0.0)}
    ...
    # obvious ground line and filled ground area
    ...
    # anchor & height (anchor is world coord)
    ax_s, ay_s = world_to_screen(*ANCHOR_W)
    ...
    # dragging preview; lower HUD so it doesn't overlap buttons
    hud_y = 60
    if dragging:
        mx, my = mouse_pos
    ...
    # live flight HUD and projectile drawing (unchanged)
    live_hud_y = 120
    if projectile and projectile['alive']:
        speed_now = math.hypot(projectile['vx'], projectile['vy'])
    ...
    # stored landings and tooltips (unchanged)
    mx, my = mouse_pos
    hover_index = None
    ...
    # temporary hover prediction on anchor when idle
