# slingshot_fixed_ui_final.py
# pip install pygame
# Run: python slingshot_fixed_ui_final.py
...
pygame.display.set_caption("Slingshot — final (fixed markers, pan, zoom, ball size)")
...
# Only horizontal camera offset allowed. Vertical offset is fixed so ground (y=0) stays.
cam_off_x_m = 0.0
cam_off_y_m = 0.0  # fixed; keeps ground fixed at HEIGHT - GROUND_H_PX
...
projectile = None
prev_state = None
landings = []  # list of dicts storing world data (x in meters etc.)
SHOT_COUNTER = 0

# interactions
dragging = False            # pulling slingshot
panning = False             # dragging the world
pan_start_mouse = (0, 0)
pan_start_cam_x = 0.0
mouse_pos = (0, 0)
last_pull = None            # last pull in world meters (pwx,pwy)
...
# UI rects for ball size controls
...
def record_landing_exact(prev, curr, anchor_x_m):
    # returns world landing x (meters), impact speed, range (m)
    y1 = prev['y']; y2 = curr['y']
    if (y2 - y1) != 0:
        frac = (0.0 - y1) / (y2 - y1)
    else:
...
while running:
    ...
    for ev in pygame.event.get():
        if ev.type == pygame.QUIT:
            ...
            # left click: either start pull (if near anchor) or start panning
            if ev.button == 1:
            ...
            if abs(new_ppm - PIXELS_PER_M) > 1e-9:
                ...
                # cam_off_y_m unchanged to keep ground fixed
    ...
    # show starting displacement box if last_pull exists
    ...
    # live flight HUD (when shot flying)
    live_hud_y = 120
    if projectile and projectile['alive']:
        speed_now = math.hypot(projectile['vx'], projectile['vy'])
    ...
    # stored landings (use world x,y and convert to screen each frame)
    mx, my = mouse_pos
    hover_index = None
    for i, ld in enumerate(landings):
        ...
        # hover detection uses the current screen pos
        if is_mouse_near(mx, my, sx, sy):
            hover_index = i

    # hover tooltip shows recorded metrics
